...
use support::{FixtureServer, Session, assert_exit, require_browser};

#[test]
fn malformed_act_and_missing_env_return_structured_errors_without_panic() {
    let Some(chrome) =
        require_browser("malformed_act_and_missing_env_return_structured_errors_without_panic")
    else {
        return;
    };
...
fn invalid_transport_and_oversized_envelope_recover_without_process_panic() {
    let Some(chrome) =
        require_browser("invalid_transport_and_oversized_envelope_recover_without_process_panic")
    else {
        return;
    };
    let directory = tempfile::tempdir().unwrap();
