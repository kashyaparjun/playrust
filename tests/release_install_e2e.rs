...
fn installs_verified_binary_from_local_tar_and_zip_fixtures() {
    ...
    header
        .set_path(std::path::Path::new("release").join(RELEASE_BINARY))
        .unwrap();
    ...
    assert_eq!(
        fs::read(tar_destination.join(RELEASE_BINARY)).unwrap(),
        payload
    );
    ...
    assert_eq!(
        fs::read(zip_destination.join(RELEASE_BINARY)).unwrap(),
        payload
    );
}
