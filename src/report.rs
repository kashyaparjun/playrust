...
mod tests {
    ...
    fn flow_report_serializes_recording_warnings_without_exposing_secrets() {
        ...
        warned.warnings = vec![
            "video/screenshots are enabled for a flow using secret-derived values; rendered page content may contain sensitive data".to_owned(),
        ];
        let report = AggregateReport::new(
            RunnerInfo {
                name: "playrust".to_owned(),
        ...
        assert!(
            decoded.flows[1].warnings[0].contains("secret-derived"),
            "{:?}",
